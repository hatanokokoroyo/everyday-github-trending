...
def get_trending_count(base_dir):
    env_path = os.path.join(base_dir, ".env")
    env_values = load_env_file(env_path)
    for key in ("TRENDING_COUNT", "TOP_N", "COUNT"):
...
def parse_trending_repos(html, limit):
...
def main():
    ...
    trending_count = get_trending_count(base_dir)

    parser = argparse.ArgumentParser()
    parser.add_argument("--output", default=f"trending_top{trending_count}.json")
    args = parser.parse_args()

    headers = build_headers()
    try:
        html = fetch_text(TRENDING_URL, headers=headers)
        repos = parse_trending_repos(html, trending_count)
